	...
	<div class="group mx-3 grid grid-cols-2 grid-cols-[4fr_1fr] divide-x-2 justify-between rounded-xl border-2 border-slate-300 transition-shadow duration-300 hover:border-purple-300 hover:shadow-lg hover:shadow-purple-300/50 lg:m-0">
		...
		<div class="py-6 px-3 mr-3">
			<div class="flex w-full flex-col items-center justify-center gap-2 ml-2 text-center">
				{#each links as link}
					<a target="_blank" href={link.url} class="rounded-lg w-full flex flex-row justify-center items-center gap-2 rounded-lg border-2 px-4 py-1 align-middle transition-colors duration-300 hover:border-purple-700 hover:bg-purple-700 hover:text-white">
							<span>{link.text}</span>
